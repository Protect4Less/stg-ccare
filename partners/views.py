...
def encode_jwt(payload, headers=None):
    ...
    # }
    # payload = calendar.timegm(datetime.datetime.utcnow().utctimetuple())
    private_key_name = settings.PROTECT4LESS_JWT_KEY.format(payload['iss'].upper())
    ...
    encoded = jwt.encode(payload, private_key, algorithm=algorithm,
                         headers=headers)
    # jwt_token = jwt.encode({'exp': payload}, key, algorithm='HS256')
    return encoded
...
def upload_create_policy(request):
    ...
    partners_config = [{"id": 1025, "name": '1025 - SAFARI HYPER MARKET - SADIQ ALI'},
                       {"id": 1026, "name": '1026 - NESTO GROUP - MR. FARHAN MOHAMED'},
                       {"id": 'RG', "name": 'RG - Redington'}, {"id": 1030, "name": '1030 - TECH-OFFER (FLORENCE TRD)'},
    ...
    if excel_file is not None and partner_code is not None:
        ...
        if len(list(worksheet.iter_rows())) > 0:
            for row in worksheet.iter_rows():
                ...
                if count < 14:
                    ...
                    break
                                                        ...
                                                        '1037', '1038', '1039','1040', '1041'] and email_value != 'None':
                    print("inside")
            ...
            print("Cell Values!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            messages.success(request, 'File Uploaded successfuly. Data will be processed') if count >= 14 else ''
        else:
            messages.warning(request, 'Document must not be blank')
    else:
        messages.info(request, 'Please chose a valid document to upload')
...
def create_policy_api(request):
    ...
    try:
        ...
        print("encoded_jwt_token",encoded_jwt_token)
        jwt_token_response = CommonLib.validate_jwt_token(encoded_jwt_token)
        print ("jwt_token_response ===>", jwt_token_response)
        error = jwt_token_response['message'] if jwt_token_response['status'] == 'NOK' else None
    except TypeError:
        error = 'Invalid Token'
    except KeyError:
        error = 'Please Pass Authorization in headers'

    print('error 1:: ', error)
    inputs = json_request['data']
    ...
    response = {}
    error = None
    # inputs = request.POST.get('data', None)
    # partner_code = '1036'
    try:
        if inputs is not None:
            cnt = 0
            cntin = 0
            # ''' no device input only device_name'''
            # device = inputs["device"]
            # ''' no sub_device input '''
            # sub_device = inputs["sub_device"]
            # ''' no model input '''
            # model = inputs["model"]
            # ''' no purchase_date input '''
            # purchase_date = inputs["purchase_date"]
            # ''' no term type input '''
            # term_type = inputs["term_type"]
            # ''' no device value input '''
            # device_value = inputs["device_value"]
            # device_name = inputs["device_name"]
            """ Location is not in api inputs"""
            location = inputs["location"]

            invoice_no = inputs["invoice_no"]
            partner_code = inputs["partner_code"]
            custom_device_name = inputs["device_name"]
            sku = inputs["sku"]
            brand = inputs["brand"]
            ...
            imei_serial_no = inputs["serial_no"]
            device_currency = inputs["plan_currency"]
            category = inputs["category"]
            model = inputs["item"]
            plan_price = inputs["plan_price"]
            plan_type = inputs["plan_type"]
            plan_tax = inputs["plan_tax"]
            plan_total_price = inputs["plan_total_price"]
            invoice_dt = inputs["invoice_dt"]

            if partner_code in ['1036'] and email != 'None':
                print("inside")
                sku_value = sku if sku is not None and sku != "None" else ""
                PartnersDAO.insert_partners_offline_policy_data({
                    'popd_partner_code': partner_code,
                    'popd_invoice_no': invoice_no,
                    'popd_plan_price': plan_price if plan_price else '',
                    'popd_plan_tax': plan_tax if plan_tax else '',
                    'popd_plan_total_price': plan_total_price if plan_total_price else '',
                    'popd_sku': sku_value if sku_value else '',
                    'popd_location': location if location else '',
                    'popd_device': category if category else '',
                    # ''' nothing in api '''
                    'popd_device_name': custom_device_name if custom_device_name  else '',
                    # ''' nothing in api '''
                    # 'popd_sub_device': sub_device_value if 'sub_device_value' in locals() else '',
                    'popd_brand': brand if brand else '',
                    'popd_model': model if model else '',
                    # '''nothing in api'''
                    # 'popd_model_code': model if model else '',
                    ''' no purchase month in api '''
                    'popd_purchase_month': invoice_dt if invoice_dt else '',
                    'popd_first_name': first_name if first_name else '',
                    'popd_last_name': lastname if lastname else '',
                    'popd_email': email if email else '',
                    'popd_mobile_number': mobile_number if mobile_number else '',
                    'popd_imei_serial_no': imei_serial_no if imei_serial_no else '',
                    'popd_term_type': plan_type if plan_type else '',
                    'popd_device_currency': device_currency if device_currency else '',
                    # 'popd_activation_date':'2021-07-16',
                    # ''' no activation date in api '''
                    'popd_activation_date': invoice_dt if invoice_dt else '',
                })

        response = {"status": "OK", "code": status.HTTP_200_OK, "message": "success",
                    "messageDesc": "Protect4Less successfully received your policy creation request. We will process it shortly."}
        jwt_token = encode_jwt(response)

        return Response(jwt_token)
    except Exception as e:
        print("Exception", e)
        response = {"status": status.HTTP_404_NOT_FOUND, "code": "201", "message": 'something went wrong',
                    "messageDesc": "",
                    "responseData": {"error_message": str(e)}
                    }
        jwt_token = encode_jwt(response)
        return Response(jwt_token)
